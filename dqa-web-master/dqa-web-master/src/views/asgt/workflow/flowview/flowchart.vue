...
export default {
  ...
  methods: {
    ...
    async loadFlowCanvas(flowData) {
      ...
      try {
        ...
        if (flowData.nodeData !==undefined && flowData.nodeData.length > 0 ) {
          self.fillColor(flowData.nodeData, flowData.activeTaskKey)
        }
      } catch (err) {
    ...
    // 设置高亮颜色的
    fillColor(nodeData, activeTaskKey) {
      ...
      this.bpmnViewer.getDefinitions().rootElements[0].flowElements.forEach(n => {
        ...
        const completeSequence = nodeData.find(m => m.key === n.id);
        if (n.$type === 'bpmn:UserTask') {
          if (completeTask) {
            if(activeTaskKey===n.id){
